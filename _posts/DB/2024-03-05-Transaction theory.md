---
title: Transaction theory
date: 2024-03-05 14:36:00 +0900
category: DB
tags: [DB, Transaction]
summary: Transaction Concept, Transaction Management, Transaction State, ACID properties, Serializability, Conflict Serializability, View Serializability, Testing for Serializability, Recoverability, Cascadeless Schedules
toc: true
toc_sticky: true
---

**Transaction**이란 데이터베이스에서 데이터를 처리하는 **(논리적인) 작업 단위**를 말합니다. 이는 데이터베이스의 일관성을 유지하면서 데이터를 안전하게 처리하기 위한 중요한 개념입니다.
...
# Transaction State
![Untitled]({{"/assets/images/transaction_theory/Untitled.png"| relative_url}}){: .align-center width="70%"}
Active, Partially committed, Committed, Aborted는 각각 트랜잭션의 상태를 나타냅니다.
- <span style="background-color:#fff5b1">Active</span>
    - 트랜잭션이 현재 실행 중이며 아직 완료되지 않았음을 나타냅니다. 이 상태에서 트랜잭션은 계속해서 데이터베이스에 대한 연산을 수행할 수 있습니다. 
- <span style="background-color:#fff5b1">Partially committed</span>
    - **트랜잭션의 모든 연산이 수행되고 커밋 명령이 나오기 직전의 상태**를 말합니다. 이 상태에서는 아직 최종 커밋이 이루어지지 않았으므로, 에러가 발생하면 롤백이 가능합니다.
- <span style="background-color:#fff5b1">Committed</span>
    - 트랜잭션의 모든 연산이 성공적으로 완료되고 데이터베이스에 영구적으로 반영된 상태를 말합니다. 이 상태가 되면 트랜잭션은 완료된 것으로 간주되며, 이후에는 롤백(취소)이 불가능합니다.
- <span style="background-color:#fff5b1">Aborted</span>
    - 트랜잭션이 실패하거나 롤백(취소)된 경우에 발생하는 상태를 말합니다. 이 상태에서는 트랜잭션이 수행되던 중에 발생한 모든 변경이 취소되고, 데이터베이스는 트랜잭션 시작 전 상태로 복구됩니다.

---

# Dealing with Issues

There are <span style="background-color:#fff5b1">two main issues</span> to address when working with **transactions**:

1. <span style="background-color:#b3cf1f">**Hardware/Software/Transaction Failures**</span>: These can occur due to various reasons such as power outages, system crashes, or errors within the transaction itself. Robust error handling and recovery procedures need to be in place to ensure the integrity of the database.
- So, a Transaction should have the **ACID** properties.
    
    Transaction이 지켜야 하는 4가지 조건 → **ACID properties**
    ...
    - <span style="background-color:#fff5b1">Atomicity</span>
    ...
    - <span style="background-color:#fff5b1">Consistency</span>
    ...
    - <span style="background-color:#fff5b1">Isolation</span>
        ...
        {: .notice}
    - <span style="background-color:#fff5b1">Durability</span>
...
2. <span style="background-color:#b3cf1f">**Concurrent Execution of Multiple Transactions**</span>: When multiple transactions are executed concurrently, conflicts can occur. Techniques like locking, timestamping, or using a concurrency control protocol can help manage these conflicts and maintain consistency in the database.
    - Then, how should we run concurrently in the system? 
        1. <span style="background-color:#fff5b1">Locking</span>: 데이터에 대한 동시 액세스를 제한하여 충돌을 방지하는 방식
        2. <span style="background-color:#fff5b1">Timestamp ordering</span>: 각 트랜잭션에 타임스탬프를 부여하고, 타임스탬프 순서대로 실행하는 방식
        3. <span style="background-color:#fff5b1">Multiversion</span>: 데이터의 여러 버전을 유지하여 동시성을 향상시키는 방식
        4. <span style="background-color:#fff5b1">Optimistic protocol</span>: 충돌이 발생할 때까지 트랜잭션을 실행하고, 충돌이 발생하면 롤백하여 다시 실행하는 방식
...
1. <span style="background-color:#fff5b1">Dirty read</span>는 트랜잭션에서 아직 **커밋되지 않은 수정 중인 데이터**를 다른 트랜잭션에서 **읽는 것**을 말합니다. 이로 인해 데이터의 불일치 문제가 발생할 수 있습니다. 
2. <span style="background-color:#fff5b1">Lost Update</span>는 두 개 이상의 트랜잭션이 **동일한 데이터를 동시에 업데이트**하려고 할 때 발생하는 문제를 의미합니다. 이로 인해 한 트랜잭션에서 수행한 업데이트가 다른 트랜잭션에 의해 덮어쓰여지거나 손실될 수 있습니다.
    
    이러한 문제를 해결하기 위해 데이터베이스 관리 시스템은 일반적으로 잠금이나 타임스탬프와 같은 동시성 제어 메커니즘을 사용합니다.
    
3. <span style="background-color:#fff5b1">Unrepeatable read</span>는 트랜잭션 내에서 동일한 데이터를 **읽을 때,** **중간에 다른 트랜잭션에 의해 해당 데이터가 수정**되어 내용이 변경되는 현상을 나타냅니다. 이로 인해 한 트랜잭션이 동일한 쿼리를 두 번 수행했을 때 결과가 일관성 없이 다르게 나타날 수 있습니다.

The **two widely-accepted criteria for serializability** in database systems are 
<span style="background-color:#b3cf1f">**conflict serializability**</span> and <span style="background-color:#b3cf1f">**view serializability**</span>
...
![Untitled]({{"/assets/images/transaction_theory/Untitled2.png"| relative_url}}){: .align-center width="60%"}

Conflict Serializable하다면 View Serializable하지만, 역은 성립하지 않습니다.
{: .notice}

![Schedule 8]({{"/assets/images/transaction_theory/Untitled1.png"| relative_url}}){: .align-center width="70%"}

위 스케줄은 View Serializable하지만 Conflict Serializable하지 않습니다.
{: .notice}

![consistent schedule]({{"/assets/images/transaction_theory/Untitled3.png"| relative_url}}){: .align-center width="70%"}

Conflict Serializability하지 않고 View Serializability하지 않아도 consistent한 경우도 있습니다.
{: .notice}
...
![cyclic graph]({{"/assets/images/transaction_theory/Untitled4.png"| relative_url}}){: .align-center width="70%"}

그래프를 그려보았을 때 사이클이 존재한다면 Not conflict serializable
{: .notice}

![uncyclic graph]({{"/assets/images/transaction_theory/Untitled5.png"| relative_url}}){: .align-center width="70%"}
...
![Untitled]({{"/assets/images/transaction_theory/Untitled6.png"| relative_url}}){: .align-center width="80%"}

RC : recoverable<br>ACA : avoids cascading aborts. **w-r만 따짐**<br>ST : strict. **w-w, w-r 둘 다 따짐**<br>SR : conflict serializable
